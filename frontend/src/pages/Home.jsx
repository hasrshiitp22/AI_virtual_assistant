...
const sleepTimer = useRef(null);
  ...
  return (
    <div className={`w-full h-screen bg-${theme} flex justify-center items-center flex-col relative`}>
      ...
      <button
        className={`absolute top-10 right-10 bg-white  border-2 border-${theme == "white" ? "black": "white"} px-8 py-2 rounded-full`}
      ...
      <button
        className={`absolute top-15 bg-white  right-10  border-2 border-${theme == "white" ? "black": "white"} px-6 py-2 rounded-full`}
        onClick={() => navigate("/customize")}
      >
        Customize
      </button>
    
      <button className={`${theme === "black" ? "bg-white text-black": "bg-black text-white"} absolute rounded-3xl px-8 py-2 rounded-full m-2 top-10 left-15`} onClick={
        ()=>{
          if(theme=='black'){
            setTheme('white');
          }else{
